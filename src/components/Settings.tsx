...
import { getSettingsDefaults } from '@/services/apiClient';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        setWeights(defaults.ecoscore_weights);
        setPue(defaults.default_pue);
        setWue(defaults.default_wue);
      } catch (e) {
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white p-4">
        <div className="flex items-start sm:items-center gap-2 text-[11px] text-slate-400">
          ...
          <span>Settings defaults are loaded from the backend. Persisting org-specific settings requires an API endpoint.</span>
        </div>
        <button
          onClick={handleSave}
          disabled={!isValid}
